...
def validation_and_prepare_input_data(
    ...
    if document_date != 'сегодня':
        try:
            ...
            raise ValueError('Дата документа(document_date) переданна не корректно, получено - %s, ожидалось - YYYY-MM-DD или сегодня.' % document_date)
    ...
    if operation_date != 'рандом':
        try:
            ...
            raise ValueError('Дата операции(operation_date) переданна не корректно, получено - %s, ожидалось - YYYY-MM-DD или рандом.' % operation_date)
...
if __name__ == '__main__':
    ...
    print(validation_and_prepare_input_data(direction='ЗАЧИСЛЕНИе'))
    ...
    #     payment_body = PaymentBDVO(direction='Зачисления')
        ...
        # payment_header = BDVOHeader('зачисления', 'зачисления')
